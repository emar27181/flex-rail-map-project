...
| 角の丸み | `L.r` | control(8) card(12) pill(999) |
...
**例外はふりがなだけ。** 駅名の読みを補うルビで、本文として読ませる文字ではないため
本文の75%にしている（下限10px）。

駅ラベルは倍率で調整できるが、既定を0.8→1.0にして規格どおりの12pxにした。
文字を大きくすると地図上でラベルが重なりやすくなる（実測で18ラベル中1組→3組）。
密度が気になる場合は設定から倍率を下げられる。
...
| 役割 | 値 | 使うところ |
|---|---|---|
| `control` | 8px | ボタン・入力欄・チップなど、操作する部品 |
| `card` | 12px | パネル・カード・ポップアップなど、部品を載せる箱 |
