 ...
 * MobileBottomPanel
 *
 * Mobile footer for the map. The information architecture is intentionally
 * limited to three user goals: route, display and settings. Search/current
 * location remain part of StationSelector because they are route inputs.
 */
...
const MobileBottomPanel: React.FC<MobileBottomPanelProps> = ({ buttons, theme }) => {
  ...
  const dragMove = useCallback((e: React.TouchEvent) => {
    ...
    setPanelHeight(Math.max(POPOVER_MIN_H, Math.min(window.innerHeight * 0.9, dragStartRef.current.startH + delta)));
  }, []);

  const orderedButtons = (['routes', 'display', 'settings'] as const)
    .map(key => buttons.find(button => button.key === key))
    .filter((button): button is FloatingButtonDef => Boolean(button));
  ...
  return <>
    {openKey && (
      <div
        aria-hidden="true"
        onClick={close}
        style={{ position: 'fixed', inset: 0, zIndex: Z_BACKDROP, background: 'rgba(0,0,0,.10)' }}
      />
    )}
    ...
    {active && (
      <section
        ref={popoverRef}
        role="dialog"
        aria-label={meta[active.key].label}
        style={{
          position: 'fixed',
          left: 10,
          right: 10,
          bottom: `calc(${NAV_H}px + env(safe-area-inset-bottom, 0px) + 8px)`,
          width: `min(${POPOVER_MAX_W}px, calc(100vw - 20px))`,
          margin: '0 auto',
          ...(panelHeight ? { height: panelHeight } : { maxHeight: POPOVER_MAX_H }),
          zIndex: Z_POPOVER,
      ...
      >
        ...
        <header style={{
          ...
          <strong style={{ display: 'flex', alignItems: 'center', gap: 8, color: colors.text, fontSize: 15 }}>
            {active.icon || meta[active.key].icon}
            {meta[active.key].label}
          </strong>
        ...
        <div
          ref={scrollRef}
          style={{ flex: 1, minHeight: 0, overflowY: 'auto', padding: `${L.sp.lg} ${L.sp['2xl']}`, overscrollBehavior: 'contain' }}
        >
          {active.content}
        </div>
      </section>
    ...
    {orderedButtons.length > 0 && (
      <nav aria-label="メイン操作" style={{
        ...
        display: 'grid', gridTemplateColumns: `repeat(${orderedButtons.length}, minmax(0, 1fr))`,
        ...
        {orderedButtons.map(button => {
          ...
          return (
            <button
              ...
              style={{
                position: 'relative', minHeight: 50, border: 0, borderRadius: 12,
                background: selected ? 'rgba(33,150,243,.12)' : 'transparent',
            ...
            >
              <span style={{ height: 22, display: 'flex', alignItems: 'center' }}>{button.icon || item.icon}</span>
              <span>{item.label}</span>
              {!!button.badge && button.badge > 0 && (
                <span style={{
                  position: 'absolute', top: 3, left: 'calc(50% + 8px)', minWidth: 16, height: 16,
                  padding: '0 4px', borderRadius: 999, background: '#2196f3', color: '#fff',
