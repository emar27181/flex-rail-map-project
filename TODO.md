...
- 路線の線をもっと正確にする
  - 現状、路線は駅間を直線で結んでいるだけで、実際の線路のカーブ（トンネル・
    高架・大きく迂回する区間など）を反映していない
  - データ源の候補: 国土交通省の GTFS-JP（`shapes.txt` に実際の走行経路の
    座標列がある）、国土数値情報の鉄道データ、OpenStreetMap の鉄道ライン
    （`railway=rail` 等、実データのみ使用すること）
  - 検討事項:
    - 現状の「駅リストの配列」というデータ構造から、駅間ごとに経路座標列を
      持たせる構造への変更が必要（既存の経路検索ロジックへの影響を要調査）
    - 全路線分のデータ量が大きくなるため、ズームレベルに応じた間引きが必要
      （今回の日本の輪郭修正で使った距離ベースの間引き手法が流用できる）
...
- バス路線の可視化を実装する
  - 鉄道が無い区間（駅から離れた住宅地・郊外）を補い、鉄道＋バスで移動を組み立てられるようにする
  - データ源の候補: 国土交通省の GTFS-JP オープンデータ、各バス事業者の公開 GTFS
    （鉄道の路線データと同じく、実データのみ使用すること）
  - 検討事項:
    - 停留所は駅より圧倒的に数が多いため、ズームに応じた間引きと表示上限が必須
    - 鉄道の駅と停留所をどう乗換として結ぶか（徒歩数分の距離判定）
    - 路線色をどう割り当てるか（事業者ごと / 系統ごと）
- 高速道路の可視化を実装する
  - 鉄道が弱い地域での移動手段として、高速道路網とインターチェンジを重ねて表示する
  - データ源の候補: OpenStreetMap（`highway=motorway`）、国土数値情報の高速道路時系列データ
  - 検討事項:
    - 鉄道路線と線種を明確に分ける（実線／破線、太さ、色）ことで路線図と混ざらないようにする
    - 表示は独立トグルにして、既定はオフ（既存の路線図の見た目を変えない）
    - インターチェンジ・ジャンクションをノードとして持つか、線だけにするか
...
- ヒートマップパラメータに「発車間隔」を追加する（過疎っている駅が浮き彫りになる）
  - データ源: `src/data/timetableData.ts` の時刻表データから算出できるが、現状98/490路線しか
    時刻表を持たないため、対応路線のみ実データ、それ以外は灰色（推定値で埋めない）
  - 算出方法の検討: 特定時間帯（例: 朝ラッシュ・日中）の平均間隔にするか、全時間帯の平均にするか
  - `StatCategory`に該当カテゴリが無いため、'transport'に含めるか新設するか検討
