...
### 🔵 バブルマップ：円サイズの地図縮尺連動
- [ ] バブルマップの円半径を「実際の地理的距離」に対応させる
  - 例: 家賃25万円 → 地図上の半径25km相当に対応（値と地理距離を等スケールで表示）
  - 現在は px 固定や zoom 依存のサイズだが、緯度経度ベースの距離で円サイズを定義する
  - Leaflet の `circle()` は半径をメートル指定できる → これを活用して実距離連動サイズを実装する
  - ズームを変えても円の地理的範囲が変わらないよう `Circle` レイヤーを使用
  - 重なり防止（パッキング）は従来通り維持する
